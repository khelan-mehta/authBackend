...
export class UsersService {
  ...
  async addOrUpdateDiary(addDiaryDto: AddDiaryDto): Promise<User | null> {
    ...
    // Custom function to format date as 'MM-dd-yyyy'
    ...
    try {
      ...
      if (existingDiaryIndex !== -1) {
        ...
        user.diaries[existingDiaryIndex].content = content;
        user.diaries[existingDiaryIndex].privacy = privacy;
        user.diaries[existingDiaryIndex].genres = genres;
        user.diaries[existingDiaryIndex].sign = this.diaryKey.privateKey;
      } else {
        // Diary for the current date does not exist, add it
        const encDiaryContent = await this.generateDiarySign(content);
        user.diaries.push({
          date: dateString,
          content: encDiaryContent,
          privacy,
          genres,
          sign: this.diaryKey.privateKey,
        });
      }
